...
if ADDON.getSetting('oldpass')=='':
    ADDON.setSetting('oldpass',ADDON.getSetting('pass'))
    
if not ADDON.getSetting('oldpass')==ADDON.getSetting('pass'):
    ADDON.setSetting('oldpass',ADDON.getSetting('pass'))
    time.sleep(2)
    LOGIN()          
...
def SUBS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://www.ntv.mx/?c=1&a=18', headers={'User-Agent' : UA}).content
    data = json.loads(link)
    body = data['body']
    for field in body:
        title= field['title']
...
def ORDERS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://www.ntv.mx/?c=1&a=19', headers={'User-Agent' : UA}).content.encode('ascii', 'ignore')
    data = json.loads(link)
    body = data['body']
    for field in body:
...
def BUYSUBS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://ntv.mx/?c=4&a=7', headers={'User-Agent' : UA}).content
    data = json.loads(link)
    for field in data:
        cat= str(field['id'])
